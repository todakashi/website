...
document.querySelectorAll('.btn-primary').forEach(button => {
    if (button.textContent.includes('Buy Now')) {
        button.addEventListener('click', function(e) {
            e.preventDefault();
            const packageName = this.closest('.account-card').querySelector('h3').textContent;
            showNotification(`Opening WhatsApp for ${packageName}...`, 'info');
            
            // Open WhatsApp with pre-filled message
            const message = `Hello, I'm interested in ${packageName}. Please provide more information.`;
...
function handleSwipe() {
    ...
    if (Math.abs(diff) > swipeThreshold) {
        if (diff > 0) {
            ...
            // Swipe down - could be used for special actions
